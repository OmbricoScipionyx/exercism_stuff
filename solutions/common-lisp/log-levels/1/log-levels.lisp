...
(defun log-severity (log-string) (let ((level (subseq log-string 1 5))) (cond ((string-equal level "info") :everything-ok)
                                                                              ((string-equal level "warn") :getting-worried)
                                                                              ((string-equal level "ohno") :run-for-cover)
                                                                               )))

(defun log-format (log-string) (cond ((eq (log-severity log-string) :everything-ok) (string-downcase (log-message log-string)))
